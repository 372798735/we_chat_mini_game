      ...
      if (task && task.id) {
        ...
        form.setFieldsValue({
          ...
          startTime: selectedDate
            ? dayjs(selectedDate).hour(9).minute(0).second(0)
            : dayjs().hour(9).minute(0).second(0),
          endTime: selectedDate
            ? dayjs(selectedDate).hour(10).minute(0).second(0)
            : dayjs().hour(10).minute(0).second(0),
          duration: 60, // 默认1小时
          // 如果有选择的日期，设置为截止日期，默认设置为选择日期的 18:00
          dueDate: selectedDate
            ? dayjs(selectedDate).hour(18).minute(0).second(0)
      ...
      >
        <Row gutter={[16, 16]}>
          ...
          <Col span={24}>
            ...
            >
              <TextArea rows={3} placeholder="请输入任务描述（可选）" showCount maxLength={500} />
            </Form.Item>
          </Col>
          ...
          <Col xs={24} sm={12}>
            <Form.Item label="任务时长（分钟）" name="duration">
              <InputNumber
                min={1}
                max={480}
                placeholder="自动计算"
          ...
          <Col xs={24} sm={12}>
            <Form.Item label="排序" name="sortOrder">
              <InputNumber min={1} max={9999} placeholder="1" style={{ width: '100%' }} />
            </Form.Item>
          </Col>

          {/* 标签选择 */}
          <Col span={24}>
            <Form.Item label="标签" name="tagIds" help="选择任务的标签，可以选择多个标签">
              <Select
                mode="multiple"
                placeholder="请选择标签"
                allowClear
              ...
              >
                {availableTags.map(tag => (
                  <Option key={tag.id} value={tag.id} label={tag.name}>
                    <Space>
                      <AntTag color={tag.color || '#1890ff'}>{tag.name}</AntTag>
                      <span>{tag.name}</span>
                    </Space>
                  </Option>
                ))}
